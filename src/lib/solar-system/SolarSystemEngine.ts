...
export class SolarSystemEngine {
    ...
    private setSelectedBody = (body: BodyRuntime | null): void => {
        ...
        if (body) {
            ...
            this.cameraOffset.copy(this.camera.position).sub(this.focusedWorldPosition)
    ...
    private updateCamera(deltaTime: number): void {
        ...
        if (this.selectedBody) {
            this.selectedBody.mesh.getWorldPosition(
                this.focusedWorldPosition,
            )
        ...
        const targetHasArrived =
            this.controls.target.distanceTo(
                this.overviewTarget,
            ) < 0.01
    ...
    private createBody = (definition: CelestialBodyDefinition): void => {
        ...
        const colorMap = definition.visual.texturePath
            ...
            : null
        const material: THREE.Material = definition.kind === "star"
            ? new THREE.MeshBasicMaterial({ color: definition.visual.color, map: colorMap })
            : new THREE.MeshStandardMaterial({ color: colorMap ? 0xffffff : definition.visual.color, map: colorMap, roughness: 0.8, metalness: 0 })
    ...
    private animate = (time: number): void => {
        ...
        const deltaTime =
            this.previousFrameTime === 0
                ? 0
                : Math.min(
                    (time - this.previousFrameTime) * 0.001,
                    0.1,
                )
